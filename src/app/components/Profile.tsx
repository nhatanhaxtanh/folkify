...
export function Profile() {
  ...
  const totalXp = instruments.reduce(
    (acc, i) => acc + i.lessons.filter((l) => l.completed).reduce((s, l) => s + l.xp, 0),
    0
  );
  ...
  const badges = [
    { icon: "🎵", name: "Nhạc sĩ mới", desc: "Hoàn thành bài học đầu tiên", earned: true },
    { icon: "🔥", name: "Kiên trì", desc: "Học 7 ngày liên tiếp", earned: true },
    { icon: "🏆", name: "Thành thạo", desc: "Hoàn thành 10 bài học", earned: false },
    { icon: "🎼", name: "Đa năng", desc: "Học 3 nhạc cụ khác nhau", earned: false },
    { icon: "⭐", name: "Xuất sắc", desc: "Đạt 100% một bài học", earned: true },
    { icon: "🎭", name: "Biểu diễn", desc: "Hoàn thành 5 bài hòa tấu", earned: false },
  ];
    ...
    <div className="flex flex-col min-h-full bg-[#F7FAF8]">
      ...
      <div className="bg-[#1A3A2B] px-5 pt-12 pb-8 relative overflow-hidden">
        ...
        <div className="flex items-center gap-2 mb-5">
          <img src={folkifyLogo} alt="Folkify" className="w-7 h-7 rounded-lg" />
          <span className="text-[#52B788] text-sm" style={{ fontWeight: 700 }}>Folkify</span>
        </div>
        ...
        <div className="relative flex flex-col items-center">
          ...
          <h1 className="text-white text-xl" style={{ fontWeight: 700 }}>Nguyễn Minh Anh</h1>
          <p className="text-[#52B788] text-sm mt-0.5">Học viên · 30 ngày</p>

          {/* Stats Row */}
          <div className="flex gap-3 mt-5 w-full">
            {[
              { label: "Bài học", value: completedLessons, icon: "📚" },
              { label: "Streak", value: streakDays, icon: "🔥" },
              { label: "XP", value: totalXp, icon: "⚡" },
            ].map((stat) => (
              <div key={stat.label} className="flex-1 bg-white/10 rounded-2xl p-3 text-center">
                <p className="text-xl mb-0.5">{stat.icon}</p>
                <p className="text-white text-lg" style={{ fontWeight: 700 }}>{stat.value}</p>
      ...
      <div className="flex-1 px-4 py-4 space-y-5">
        ...
        >
          ...
          <div className="flex-1 text-left">
            <p className="text-[#1A3A2B] text-sm" style={{ fontWeight: 700 }}>Subscription</p>
            <p className="text-gray-500 text-xs mt-0.5">
              Gói hiện tại: <span style={{ fontWeight: 700 }}>{isPremium ? "Premium" : "Free"}</span>
            </p>
          </div>
        ...
        {activeTab === 0 && (
          <>
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
              <p className="text-[#1A3A2B] text-sm mb-4" style={{ fontWeight: 700 }}>
                📊 Hoạt động tuần này
              </p>
              <div className="flex items-end gap-2 h-24">
                {weekDays.map((day, i) => {
                  const height = studyData[i] === 0 ? 4 : (studyData[i] / maxStudy) * 80;
                  const isToday = i === 4;
                  return (
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
              ...
              <div className="space-y-3">
                {instruments.map((inst) => {
                  ...
                  const instXp = inst.lessons.filter((l) => l.completed).reduce((s, l) => s + l.xp, 0);
                  return (
                    <div key={inst.id}>
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          <span>{inst.emoji}</span>
                          <span className="text-gray-700 text-xs" style={{ fontWeight: 600 }}>{inst.name}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          {instXp > 0 && (
                            <div className="flex items-center gap-0.5">
                              <Zap size={10} className="text-yellow-500" />
                              <span className="text-yellow-600 text-[10px]" style={{ fontWeight: 600 }}>{instXp} XP</span>
                            </div>
                          )}
                          <span className="text-gray-400 text-[10px]">{done}/{inst.lessons.length}</span>
                        </div>
                      </div>
            ...
            <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl p-4 border border-orange-100">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-orange-800 text-sm" style={{ fontWeight: 700 }}>🔥 {streakDays} ngày liên tiếp!</p>
                  <p className="text-orange-600 text-xs">Hãy giữ vững phong độ</p>
                </div>
              </div>
              ...
              <p className="text-orange-500 text-[11px] mt-2">14 ngày gần nhất</p>
            </div>
          </>
        )}
              ...
              {badges.map((badge, i) => (
                <div
                  ...
                  className={`bg-white rounded-2xl p-4 shadow-sm border flex flex-col items-center gap-2 text-center ${
                    badge.earned ? "border-[#52B788]/40" : "border-gray-100 opacity-50"
                  }`}
                >
                  <div
                  ...
                  <div>
                    <p className="text-[#1A3A2B] text-xs" style={{ fontWeight: 700 }}>{badge.name}</p>
                    <p className="text-gray-400 text-[10px] mt-0.5 leading-relaxed">{badge.desc}</p>
                  </div>
                  {badge.earned && (
          ...
          <>
            {menuItems.map((section) => (
              <div key={section.section}>
                <p className="text-gray-400 text-xs mb-2 px-1 uppercase" style={{ fontWeight: 600, letterSpacing: "0.05em" }}>
                  {section.section}
                </p>
                  ...
                  {section.items.map((item, i) => (
                    ...
                    >
                      ...
                      >
                        <item.icon
                          size={16}
                          className={(item as any).danger ? "text-red-500" : "text-[#2D6A4F]"}
                        />
                      </div>
                      <p
            ...
            <div className="text-center py-4">
              <div className="flex items-center justify-center gap-2 mb-1">
                <img src={folkifyLogo} alt="Folkify" className="w-6 h-6 rounded-md" />
                <span className="text-gray-400 text-xs" style={{ fontWeight: 600 }}>Folkify v1.0.0</span>
              </div>
              <p className="text-gray-300 text-xs">Bảo tồn văn hóa âm nhạc truyền thống Việt Nam</p>
            </div>
          </>
        )}
      </div>
